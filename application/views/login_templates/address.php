...
console.log(alldata.total);
    ...
    $scope.updatecart=function(rowid,index){
    //alert($scope.qty[index]);
    
		var request = $http({
                    method: "post",
                    url: "<?php echo site_url('store/cart_update');?>",
                    data: {
                        updatetype: "check",
						row_id: rowid,
                    	qty: $scope.qty[index],
                 ...
                 $scope.names = response.data.product_info;
				 $scope.total = response.data.total;
					$scope.shipping = response.data.shipping;
					$scope.product_total = response.data.product_total;
				 console.log($scope.names);
       					 });
